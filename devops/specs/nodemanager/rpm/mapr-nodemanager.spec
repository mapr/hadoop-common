...
summary:     Apache Hadoop Nodemanager distribution included in HPE DataFabric Software Ecosystem Pack
...
%description
Apache Hadoop Nodemanager distribution included in HPE DataFabric Software Ecosystem Pack
...
if [ "$1" = "0" ]; then
    ...
    sudo su -c "__INSTALL_3DIGIT__/bin/yarn --daemon stop nodemanager" -s /bin/sh $daemon_user
    rm -f __PREFIX__/conf/conf.d/warden.nodemanager.conf
fi
