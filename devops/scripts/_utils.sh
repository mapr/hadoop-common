...
setup_role_package() {
  ...
  pkg_home="${BUILD_ROOT}/root/${package_name}${PKG_INSTALL_ROOT}"
  mkdir -p "${opt_mapr}/roles"
  mkdir -pv "${pkg_home}/ext-conf"
  echo -e "PKG_HOME_DIR=${PKG_INSTALL_ROOT}\nPKG_CONFIG_COMMAND=${PKG_INSTALL_ROOT}/bin/configure.sh\n" > "${opt_mapr}/roles/${package_name}"
  if [ "${package_name}" = "timelineserver" ]; then
    mkdir -p "${pkg_home}/etc/hadoop"
	  cp "${WORK_DIR}/ext-conf/warden.timelineserver.conf" "${pkg_home}/ext-conf"
	  cp "${WORK_DIR}/ext-conf/yarn-timelineserver-properties.xml" "${pkg_home}/etc/hadoop"
	  cp "${WORK_DIR}/ext-conf/yarn-timelineserver-security-properties.xml" "${pkg_home}/etc/hadoop"
  elif [ "${package_name}" = "timelineserverv1" ]; then
    mkdir -p "${pkg_home}/etc/hadoop"
	  cp "${WORK_DIR}/ext-conf/warden.timelineserverv1.conf" "${pkg_home}/ext-conf/"
	  cp "${WORK_DIR}/ext-conf/yarn-timelineserver-properties-v1.xml" "${pkg_home}/etc/hadoop/yarn-timelineserver-properties.xml"
	  cp "${WORK_DIR}/ext-conf/yarn-timelineserver-security-properties.xml" "${pkg_home}/etc/hadoop"
  fi
  cp "${WORK_DIR}/ext-conf/warden.${package_name}.conf" "${pkg_home}/ext-conf"
  sed -i 's|__HADOOP_VERSION__|'${PKG_3DIGIT_VERSION}'|g' "${pkg_home}/ext-conf/warden.${package_name}.conf"
  mkdir -p "${opt_mapr}/servicesconf"
  echo "hadoop/hadoop-${PKG_3DIGIT_VERSION}/etc/hadoop" >> "${opt_mapr}/servicesconf/${package_name}"
}
...
_build_deb() {
  ...
  mkdir -p "$DIST_DIR"
  dpkg-deb -Zxz --build "$deb_root" "$DIST_DIR"
}
