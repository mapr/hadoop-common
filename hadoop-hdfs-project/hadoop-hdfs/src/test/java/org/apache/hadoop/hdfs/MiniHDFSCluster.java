...
public class MiniHDFSCluster extends MiniDFSCluster {

  private static final String NAMESERVICE_ID_PREFIX = "nameserviceId";
  private static final Log LOG = LogFactory.getLog(MiniHDFSCluster.class);
  ...
  /**
   * Used by builder to create and return an instance of MiniHDFSCluster
   */
  protected MiniHDFSCluster(Builder builder) throws IOException {
    ...
    initMiniHDFSCluster(builder.conf,
   ...
   * multiple MiniHDFSClusters are used in the same test suite.
   */
   ...
   */
  public MiniHDFSCluster() {
    nameNodes = new NameNodeInfo[0]; // No namenode in the cluster
    synchronized (MiniHDFSCluster.class) {
   ...
   */
  @Deprecated // in 22 to be removed in 24. Use MiniHDFSCluster.Builder instead
  public MiniHDFSCluster(Configuration conf,
                        int numDataNodes,
   ...
   */
  @Deprecated // in 22 to be removed in 24. Use MiniHDFSCluster.Builder instead
  public MiniHDFSCluster(int nameNodePort,
                        Configuration conf,
                        int numDataNodes,
                        boolean format,
  ...
  public MiniHDFSCluster(int nameNodePort,
    ...
    initMiniHDFSCluster(conf, numDataNodes, format,
  ...
  private void initMiniHDFSCluster(
      Configuration conf,
  ...
  throws IOException {
    ...
    if (!nnTopology.allHttpPortsSpecified() &&
        nnTopology.isHA()) {
      LOG.info("MiniHDFSCluster disabling checkpointing in the Standby node " +
    ...
    if (!nnTopology.allIpcPortsSpecified() &&
        nnTopology.isHA()) {
      LOG.info("MiniHDFSCluster disabling log-roll triggering in the "
  ...
  /**
   * @return URI of the namenode from a single namenode MiniHDFSCluster
   */
  public URI getURI() {
    checkSingleNameNode();
    return getURI(0);
  ...
  /**
   * @return URI of the given namenode in MiniHDFSCluster
   */
  ...
  /**
   * @see MiniHDFSCluster#startDataNodes(Configuration, int, boolean, StartupOption,
   ...
   * @see MiniHDFSCluster#finalizeCluster(Configuration)
   * @param nnIndex
   * @param conf
   * @throws Exception
   */
  ...
  public synchronized DataNodeProperties stopDataNode(int i) {
    ...
    LOG.info("MiniHDFSCluster Stopping DataNode " +
                       dn.getDisplayName() +
   ...
   */
  public static void shutdownCluster(MiniHDFSCluster cluster) {
    if (cluster != null) {
      cluster.shutdown();
    }
  }
  ...
  public static File getBlockFile(int dnIndex, ExtendedBlock block) {
    ...
    for (int i = 0; i <=1 ; i++) {
      File storageDir = MiniHDFSCluster.getStorageDir(dnIndex, i);
  ...
  /**
   * Throw an exception if the MiniHDFSCluster is not started with a single
   * namenode
   */
