...
public class TestHSync {
  ...
  private void checkSyncMetric(MiniHDFSCluster cluster, int dn, long value) {
    DataNode datanode = cluster.getDataNodes().get(dn);
    assertCounter("FsyncCount", value, getMetrics(datanode.getMetrics().name()));    
  }
  private void checkSyncMetric(MiniHDFSCluster cluster, long value) {
  ...
  public void testHSyncWithReplication() throws Exception {
    Configuration conf = new HdfsConfiguration();
    MiniHDFSCluster cluster = new MiniDFSCluster.Builder(conf).numDataNodes(3).buildHDFS();
    final FileSystem fs = cluster.getFileSystem();
