...
| `yarn.scheduler.fair.resources-based-on-labels-enabled` | Whether computing of resources based on node labels is enabled. Defaults to false. |
...
| `yarn.scheduler.fair.preemption.cluster-utilization-threshold.based-on-labels-enabled` | Whether preemption utilization threshold based on node labels is enabled. Preemption can happen when the threshold of using some resource is exceeded in the nodes that have the same label. Defaults to false. |
...
* **A defaultQueueLabel element**: which sets the default label for queues which do not have any label; . All queues (exclude "root") which do not have a label will get "defaultQueueLabel" as their label.
