...
public class FairScheduler extends
  ...
  protected void preemptResources(Map<FSAppAttempt, Resource> toPreempt) {
    ...
    try {
      ...
      for (FSLeafQueue queue : getQueueManager().getLeafQueues()) {
        ...
        queue.resetNonEligibleForPreemptionSet(); //TODO: Implement this in queue
      }
  ...
  private void updateResourceToPreempt(RMContainer container,
    ...
    if (preemptApp == null) {
      LOG.info("Preempt application list doesn't have container: " + container.getContainerId());
    }
    if (toPreemptResource == null && preemptApp != null) {
      LOG.info("Don't find " + preemptApp.getName() + " at toPreempt list");
    }
    if (containerResource == null) {
      LOG.info("Container " + container.getContainerId() + " resource equals null");
    }
