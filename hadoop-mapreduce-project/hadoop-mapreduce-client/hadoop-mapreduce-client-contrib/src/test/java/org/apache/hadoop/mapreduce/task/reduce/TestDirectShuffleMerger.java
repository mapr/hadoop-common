...
public class TestDirectShuffleMerger {
  ...
  public void cleanup() throws IOException {    
   // fs.delete(new Path(jobConf.getLocalDirs()[0]), true);
    fs.delete(new Path("/tmp/abc1"), true);
  }
  ...
  public void testInMemoryMerger() throws Throwable {
    ...
    DirectShuffleMergeManagerImpl<Text, Text> mergeManager = new DirectShuffleMergeManagerImpl<Text, Text>(
        reduceId, jobConf, fs, Reporter.NULL, null, null, null, null, null,
        null, null, new Progress(), mof);
    ...
    DirectInMemoryOutput<Text, Text> mapOutput1 = new DirectInMemoryOutput<Text, Text>(
        conf, mapId1, mergeManager, mapOutputBytes1.length, null, true);
    DirectInMemoryOutput<Text, Text> mapOutput2 = new DirectInMemoryOutput<Text, Text>(
    ...
    MergeThread<DirectInMemoryOutput<Text, Text>, Text, Text> inMemoryMerger =
        mergeManager.createInMemoryMerger();
    List<DirectInMemoryOutput<Text, Text>> mapOutputs =
        new ArrayList<DirectInMemoryOutput<Text, Text>>();
    mapOutputs.add(mapOutput1);
    mapOutputs.add(mapOutput2);
    
    when(mof.getInputFileForWrite(mapId1.getTaskID(), 103)).thenReturn(new Path("/tmp/abc1"));
    when(mof.getInputFileForWrite(mapId2.getTaskID(), 0)).thenReturn(new Path("/tmp/abc2"));
    
    
    inMemoryMerger.merge(mapOutputs);
    
    Assert.assertEquals(1, mergeManager.onDiskMapOutputs.size());
    Path outPath = mergeManager.onDiskMapOutputs.iterator().next().getPath();
