...
function UpdateFileClientConfig() {
    ...
    if [ -e "$fairSchedulerConf" ] && ! grep -q "aclSubmitApps" "$fairSchedulerConf"; then
        sed -i -e "s|</allocations>|  <queue name=\"root\">\n    <aclSubmitApps>*</aclSubmitApps>\n    <aclAdministerApps> </aclAdministerApps>\n  </queue>\n</allocations>|" "$fairSchedulerConf"
    fi

    # edit capacity-scheduler.xml to add Acls
    if [ -e "$capacitySchedulerConf" ] && ! grep -q "yarn.scheduler.capacity.root.acl_submit_applications" "$capacitySchedulerConf"; then
        sed -i -e "s|</configuration>|  <property>\n   <name>yarn.scheduler.capacity.root.acl_submit_applications</name>\n    <value>*</value>\n    <description>\n      The ACL of who can submit jobs to the root queue.\n    </description>\n  </property>\n</configuration>|" "$capacitySchedulerConf"
    fi

    if [ -e "$capacitySchedulerConf" ] && ! grep -q "yarn.scheduler.capacity.root.acl_administer_queue" "$capacitySchedulerConf"; then
    ...
    if [ -e "$capacitySchedulerConf" ] && grep -q "yarn.scheduler.capacity.root.default.acl_administer_queue" "$capacitySchedulerConf"; then
...
function ConfigureHS() {
    ...
    if [ ! -z "$HS_IP" ]; then
        ...
        if [ $(grep "__HS_IP__" "${HADOOP_HOME}/etc/hadoop/mapred-site.xml" | wc -l) -ne 0 ]; then
            ...
            sed -i "s/__HS_IP__/${HS_IP}/g" "${FILE}"
        else
...
function checkAndConfigureFIPSProperties() {
  isFipsConfigured
  if [ "$isFips" == "true" ] && [ -f ${HADOOP_HOME}/etc/hadoop/yarn-site.xml ];then
      if ! grep -q "hadoop.security.token.authentication.method" "${HADOOP_HOME}/etc/hadoop/core-site.xml"; then
