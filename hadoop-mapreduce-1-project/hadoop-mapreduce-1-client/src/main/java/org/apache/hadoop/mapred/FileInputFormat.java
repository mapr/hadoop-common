...
public abstract class FileInputFormat<K, V> implements InputFormat<K, V> {
  ...
  private static final ThreadFactory DAEMON_THREAD_FACTORY = new ThreadFactory() {
    ...
    public Thread newThread(Runnable r) { 
      ...
      thread.setName("getSplits-" + counter.getAndIncrement());
      return thread;
    }    
  };
  ...
  public InputSplit[] getSplits(JobConf job, int numSplits)
    ...
    final long goalSize = totalSize / (numSplits == 0 ? 1 : numSplits);
    final long minSize = Math.max(job.getLong("mapred.min.split.size", 1),
                            minSplitSize);

    // generate splits
    ArrayList<FileSplit> splits = new ArrayList<FileSplit>(numSplits);
    final NetworkTopology clusterMap = new NetworkTopology();
    int currentMaxThreads = job.getInt("mapred.maxthreads.generate.splits", 5);
        ...
        public List<FileSplit> call() throws Exception {
          ...
          if ( length == 0 ) {
            ...
            if (isSplitable(fs, path)) { 
              ...
              retList.add(makeSplit(path, 0, length, splitHosts));
              return retList;
            } 
         }
        }
