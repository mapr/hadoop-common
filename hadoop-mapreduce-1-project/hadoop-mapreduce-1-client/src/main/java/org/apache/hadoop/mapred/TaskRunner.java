...
abstract class TaskRunner extends Thread {
  ...
  private String diagnosticsDir = null;

  private final String taskUser;
  private final String taskUserGroup;
   ...
   */
  protected MapReduceLocalData mapOutputFile;

  public TaskRunner(TaskTracker.TaskInProgress tip, TaskTracker tracker, 
                    JobConf conf, TaskTracker.RunningJob rjob
                    ) throws IOException {
    taskUser = rjob.ugi.getShortUserName();
    final String[] groupNames = rjob.ugi.getGroupNames();
    taskUserGroup = groupNames.length > 0 ? groupNames[0] : null;
    this.tip = tip;
    this.t = tip.getTask();
    this.tracker = tracker;
    this.conf = conf;
    if (TaskTracker.useMapRFs(conf)) {
      final MapRFsOutputFile mof = new MapRFsOutputFile(t.getJobID());
      mof.setUser(taskUser, taskUserGroup);
      this.mapOutputFile = mof;
   ...
   */
  @Deprecated
  public long getChildUlimit(JobConf jobConf) {
    return jobConf.getLong(JobConf.MAPRED_TASK_ULIMIT, -1);
  }
  ...
  public final void run() {
    ...
    try {
      ...
      setupCmds.add(setup);
      launchJvmAndWait(setupCmds, mapRSetupCmds, vargs, stdout, stderr, 
                       logSize, workDir);
  ...
  void launchJvmAndWait(List<String> setup, Map <String, String> mapRSetupCmds, 
      Vector<String> vargs, File stdout,
      File stderr, long logSize, File workDir)
      throws InterruptedException, IOException {
    jvmManager.launchJvm(this, jvmManager.constructJvmEnv(setup, mapRSetupCmds, vargs, 
  ...
  File[] prepareLogFiles(TaskAttemptID taskid, boolean isCleanup)
    ...
    CentralTaskLogUtil.prepareLogFiles(
      logFiles[0].getParentFile().toString(),
      taskUser,
      taskUserGroup);
  ...
  private String getVMSetupCmd() {
    final long ulimit = getChildUlimit(conf);
    // If task is not ephemeral then check if ulimit is set in job conf.
  ...
  private Vector<String> getVMArgs(TaskAttemptID taskid, File workDir,
    ...
    for (int i = 0; i < javaOptsSplit.length; i++) {
      /* Add only if its not empty */
      if (!javaOptsSplit[i].isEmpty())
    ...
    //MapR pass through security settings
    passJVMProperties(vargs, "java.security.auth.login.config");
    passJVMProperties(vargs, "hadoop.login");
    ...
    if (conf.getProfileEnabled() && !t.isTaskCleanupTask()) {
  ...
  private void setupLog4jProperties(Vector<String> vargs, TaskAttemptID taskid,
    ...
    vargs.add("-Dhadoop.root.logger=" + getLogLevel(conf).toString() + ","
      + (CentralTaskLogUtil.CENTRAL_LOG ? "maprfsTLA" : "TLA"));
  ...
  /**
    helper to pass through jvm properties
  */
  static void passJVMProperties(List<String> command, String property) {
  ...
  private String getVMEnvironment(String errorInfo, File workDir, 
    ...
    /* getting java.library.path as a superset of LD_LIBRARY_PATH for ljvm */
    oldLdLibraryPath = System.getProperty("java.library.path");
    ...
    String userTicketFile = conf.get("mapreduce.job.userticket");
    if ( userTicketFile != null ) {
      if ( LOG.isDebugEnabled()) {
        LOG.debug("putting userticket into environment " + userTicketFile);
      }
      env.put(UserGroupInformation.USER_TICKET_FILE_LOCATION, userTicketFile);
    } 
    
    // for the child of task jvm, set hadoop.root.logger
    env.put(  "HADOOP_ROOT_LOGGER", "INFO,"
            + (CentralTaskLogUtil.CENTRAL_LOG ? "maprfsTLA" : "TLA"));
  ...
  static void setupChildMapredLocalDirs(Task t, JobConf conf) {
    ...
    conf.set(JobConf.MAPRED_LOCAL_DIR_PROPERTY, childMapredLocalDir.toString());
  }
    ...
    if (DistributedCache.getSymlink(conf)) {
      ...
      if (archives != null) {
        for (int i = 0; i < archives.length; i++) {
          ...
          symlink(conf, workDir, target, link);
        }
      }
      if (files != null) {
  ...
  private static void symlink
    (JobConf conf, File workDir, String target, String link)
  throws IOException
  {
    if (link != null) {
   ...
   * @throws IOException 
   */
  public void kill(boolean killNow) throws IOException, InterruptedException {
    killed = true;
    jvmManager.taskKilled(this, killNow);
    signalDone();
  }
  ...
  // collect diagnostics for this task before killing it
  public void collectDiagnostics(String diagnosticsDir) {
