...
class SpillRecord {
  ...
  throws IOException {
    if (TaskTracker.useMapRFs(job)) {
      this.rfs = FileSystem.get(job);
      readFromMapRFile(indexFileName, job);
    } else {
      this.rfs = FileSystem.getLocal(job).getRaw();
      readFromFile(indexFileName, job, new PureJavaCrc32(), expectedIndexOwner);
    }
  }

  private void readFromMapRFile(Path indexFileName, JobConf job)
    throws IOException {
  ...
  public void readFromFile(Path indexFileName, JobConf job, Checksum crc,
