 ...
 */
public class LinuxTaskController extends TaskController {
  ...
  @Override
  public void initializeJob(String user, String jobid, Path credentials, Path userCredentials,
    ...
    if ( userCredentials != null ) {
      command.add(userCredentials.toUri().getPath().toString());
    }
    ...
    //command.add("-Dhadoop.root.logger=DEBUG,DRFA");
    //command.add("-Dhadoop.log.file=hadoop-mapr-taskcontroller.log");
    TaskRunner.passJVMProperties(command, "java.security.auth.login.config");
    TaskRunner.passJVMProperties(command, "hadoop.login");
  ...
  public int launchTask(String user, 
    ...
    try {
      ...
      // get the JVM command line.
       String cmdLine = 
        TaskLog.buildCommandLine(setup, setupCmds, jvmArguments,
      ...
      String[] command = 
        new String[]{taskControllerExe, 
          ...
          commandFile};
      shExec = new ShellCommandExecutor(command, null, env);
  ...
  public void deleteAsUser(String user, String subDir) throws IOException {
    String[] command = 
      new String[]{taskControllerExe, 
                   ...
                   subDir};
    ShellCommandExecutor shExec = new ShellCommandExecutor(command, null, env);
    if (LOG.isDebugEnabled()) {
    ...
    try {
      ...
      if (shExec != null) {
        int exitCode = shExec.getExitCode();
        if (exitCode != 143 && exitCode != 137) {
          LOG.warn("Exception thrown while deleting user dir " + Arrays.toString(command) 
                   + StringUtils.stringifyException(e));
          LOG.info("Output from LinuxTaskController's deleteAsUser follows:");
          logOutput(shExec.getOutput());
        }
      }
    ...
    try {
      ...
      if (shExec != null) {
        ...
        if (exitCode != 143 && exitCode != 137) {
          LOG.warn("Exception thrown while deleting user dir " + Arrays.toString(command) 
                   + StringUtils.stringifyException(e));
          LOG.info("Output from LinuxTaskController's deleteLogAsUser follows:");
          logOutput(shExec.getOutput());
        }
      }
      throw new IOException(e);
  ...
  public void truncateLogsAsUser(String user, List<Task> allAttempts)
    ...
    ShellCommandExecutor shExec =
      new ShellCommandExecutor(taskControllerCmd, null, env);
    try {
      shExec.execute();
    } catch (Exception e) {
  ...
  @Override
  public void runAsUser(String user, String[] cmd) throws IOException {
    int i = 0;
    final String[] tcCmd = new String[3 + cmd.length];
    tcCmd[i++] = taskControllerExe;
    tcCmd[i++] = user;
    tcCmd[i++] = Integer.toString(Commands.RUN_COMMAND_AS_USER.getValue());
    System.arraycopy(cmd, 0, tcCmd, i, cmd.length);

    final ShellCommandExecutor shExec =
      new ShellCommandExecutor(tcCmd, null, env);
