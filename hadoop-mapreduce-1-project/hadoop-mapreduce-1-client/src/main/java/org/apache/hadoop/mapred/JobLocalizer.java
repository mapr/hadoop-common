...
public class JobLocalizer {
  ...
  private InetSocketAddress ttAddr;
  private static UserGroupInformation ugiJob;
  ...
  public JobLocalizer(JobConf ttConf, String user, String jobid, InetSocketAddress ttAddr)
      throws IOException {
    this(ttConf, user, jobid, ttAddr, 
        ttConf.getLocalDirs());
  }

  public JobLocalizer(JobConf ttConf, String user, String jobid)
  throws IOException {
    this(ttConf, user, jobid, null,
        ttConf.getLocalDirs());
}
  ...
  public JobLocalizer(JobConf ttConf, String user, String jobid, 
    ...
    USERTICKET = JOBDIR + "/" + TaskTracker.USER_TICKET_FILE;
  }
  ...
  public void localizeJobFiles(JobID jobid, JobConf jConf,
    ...
    localizeJobFiles(jobid, jConf,
        lDirAlloc.getLocalPathForWrite(JOBCONF, ttConf), localJobTokenFile, taskTracker); 
  }

  public void localizeJobFiles(final JobID jobid, JobConf jConf,
      Path localJobFile, Path localJobTokenFile, TaskUmbilicalProtocol taskTracker) 
  throws IOException, InterruptedException {
    Path localUserTicketFile = null;
    
    if ( UserGroupInformation.isSecurityEnabled()) {
      localUserTicketFile = lDirAlloc.getLocalPathToRead(USERTICKET, ttConf);
    ...
    if ( localUserTicketFile != null ) {
      jConf.set("mapreduce.job.userticket", localUserTicketFile.toString());
    }
    ...
    if (sizes != null) {
      if ( ttAddr != null ) {
        final TaskUmbilicalProtocol tasktracker = (taskTracker != null ) ? taskTracker :
        ...
        UserGroupInformation ugiJobLocal = ugiJob;
        if ( ugiJobLocal == null ) {
          if ( LOG.isDebugEnabled() ) {
            LOG.debug("WARN: Creates new remoteUser");
          }
         ugiJobLocal = UserGroupInformation.createRemoteUser(jobid.toString());
        }
        ugiJobLocal.doAs(new PrivilegedExceptionAction<Object>() { 
  ...
  public Path findUserTicket() throws IOException {
    return lDirAlloc.getLocalPathToRead(USERTICKET, ttConf);
  }

  public int runSetup(String user, String jobid, Path localJobTokenFile) throws IOException,
  ...
  public static void main(String[] argv)
    ...
    if ( LOG.isDebugEnabled() ) {
      LOG.debug("Start of JobLocalizer: " + Arrays.asList(argv));
    }
    ...
    final JobLocalizer localizer =
      new JobLocalizer(conf, user, jobid, ttAddr);
    final Path jobTokenFile = localizer.findCredentials();
    
    UserGroupInformation ugi = UserGroupInformation.createRemoteUser(user);
    ugiJob = UserGroupInformation.createRemoteUser(jobid);
    
    if ((jobTokenFile != null) && UserGroupInformation.isSecurityEnabled()) {
    ...
    System.exit(
      ugi.doAs(new PrivilegedExceptionAction<Integer>() {
        public Integer run() {
          try {
            return localizer.runSetup(user, jobid, jobTokenFile); 
          } catch (Throwable e) {
