...
public class TaskLogsTruncater {
  ...
  public boolean shouldTruncateLogs(JVMInfo lInfo) throws IOException {
    ...
    for (LogName logName : LogName.values()) {
      ...
      if (CentralTaskLogUtil.existsFile(logFile)) {
  ...
  public void truncateLogs(JVMInfo lInfo) {
    ...
    OutputStream tmpFileOutputStream;
    ...
    logNameLoop: for (LogName logName : LogName.values()) {
      ...
      try {
        ...
        tmpFileOutputStream = CentralTaskLogUtil.CENTRAL_LOG ?
          CentralTaskLogUtil.createNewOutputStream(tmpFile.toString(), umask) :
          SecureIOUtils.createForWrite(tmpFile, umask);
      ...
      try {
        ...
        logFileInputStream =
          CentralTaskLogUtil.getInputStream(logFile.toString());
      } catch (IOException ioe) {
        ...
        if (!CentralTaskLogUtil.deleteFile(tmpFile, false)) {
      ...
      if (!CentralTaskLogUtil.renameFile(tmpFile, logFile)) {
    ...
    if (indexModified && !CentralTaskLogUtil.CENTRAL_LOG) {
  ...
  private Map<Task, Map<LogName, LogFileDetail>> getAllLogsFileDetails(
    ...
    for (Task task : allAttempts) {
      ...
      allLogsFileDetails = TaskLog.getAllLogsFileDetails(
        task.getTaskID(),
  ...
  private boolean isTruncationNeeded(JVMInfo lInfo,
    ...
    for (Task task : lInfo.getAllAttempts()) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("Considering truncation task=" + task + " log=" + logName
                + " size=" + logFileDetail.length + " retainSize="
                + taskRetainSize);
      }
  ...
  private LogFileDetail truncateALogFileOfAnAttempt(
      ...
      final OutputStream tmpFileOutputStream,
      final InputStream logFileInputStream, final LogName logName)
      throws IOException {
