...
public class Counters implements Writable, Iterable<Counters.Group> {
  ...
  private static final char[] charsToEscape =  {GROUP_OPEN, GROUP_CLOSE, 
  ...
  public class Group implements Writable, Iterable<Counter> {
    ...
    private final Map<String, Counter> subcounters = new HashMap<String, Counter>();
  ...
  private final Map<String,Group> counters = new HashMap<String, Group>();

  /**
   * A cache from enum values to the associated counter. Dramatically speeds up
   * typical usage.
   */
  private final Map<Enum, Counter> cache = new IdentityHashMap<Enum, Counter>();
