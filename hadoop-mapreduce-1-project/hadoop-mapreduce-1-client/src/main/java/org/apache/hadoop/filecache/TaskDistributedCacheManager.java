...
public class TaskDistributedCacheManager {
  ...
  public void setupCache(Configuration taskConf, String publicCacheSubdir,
    ...
    for (CacheFile cacheFile : cacheFiles) {
      ...
      Path cacheFilePath = new Path(uri.getPath());
      if (!cacheFilePath.isAbsolute()) {
        cacheFilePath =
          new Path(FileSystem.get(taskConf).getHomeDirectory(), cacheFilePath);
        uri = cacheFilePath.toUri();
      }
      FileStatus fileStatus = fileSystem.getFileStatus(cacheFilePath);
      Path p;
