...
class MapTask extends Task {
  ...
  public void run(final JobConf job, final TaskUmbilicalProtocol umbilical) 
    ...
    this.umbilical = umbilical;
    if (LOG.isDebugEnabled()) {
      LOG.debug("Starting task reporter");
    }
  ...
  private void updateJobWithSplit(final JobConf job, InputSplit inputSplit) {
    if (inputSplit instanceof FileSplit) {
      ...
      if (LOG.isInfoEnabled())
        LOG.info("Split Details: File = " + fileSplit.getPath().toString() +
  ...
  private static class OldOutputCollector<K,V> implements OutputCollector<K,V> {
    ...
    public void collect(K key, V value) throws IOException {
      try {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug("InterruptedException: restoring interrupted status", ie);
        }
  ...
  implements MapOutputCollector<K, V> {
    ...
    private final int keyPrefixLen;    // key prefix length in bytes
    ...
    private final int keyStart;              // key offset in acct
    private final int valStart;              // val offset in acct
    private final int acctSize;              // total #fields in acct
    private final int recSize;               // acct bytes per record
    ...
    public MapOutputBuffer(TaskUmbilicalProtocol umbilical, JobConf job,
      ...
      //sanity checks
      final float spillper = job.getFloat("io.sort.spill.percent", -1f);
      final float recper = job.getFloat("io.sort.record.percent", -1f);
      int sortmb = 100;
      if (Runtime.getRuntime().maxMemory() >= MapRConf.IO_SORT_XMX_THRESHOLD) {
        sortmb = Integer.valueOf(MapRConf.IO_SORT_MB);
      }
      sortmb = job.getInt("io.sort.mb", -1);

      /* Bug 3711 Disable auto calculation of io.sort.mb
      if (sortmb == -1) {
        // min (blocksize * 1.3, 50% map's xmx);
        sortmb = (int) (Runtime.getRuntime().maxMemory()/(2*1024*1024));
      }
      */
      ...
      // align recordCapacity by partitions
      ...
      if (LOG.isInfoEnabled()) {
        ...
        LOG.info("record buffer = " + softRecordLimit + "/" + kvoffsetTotal);
      }
      // k/v serialization
    ...
    private void initKvOffsets() {
      kvoffsets = new MapOutputBuffer.KvOffset[partitions];
      for (int i = 0; i < partitions; i++) {
    ...
    public synchronized void flush() throws IOException, ClassNotFoundException,
      ...
      boolean done = false;
      if (LOG.isDebugEnabled()) {
        LOG.debug("Starting flush of map output");
      }
      spillLock.lock();
      try {
        while (kvstart != kvend) {
        ...
        if (kvend != kvindex) {
          kvend = kvindex;
          bufend = bufmark;
          if (useMapRFs && numSpills == 0) {
            if ( LOG.isDebugEnabled()) {
              LOG.debug("It is MaprFS and no spills");
            }
            ...
            if ( LOG.isDebugEnabled()) {
              LOG.debug("There are spills: " + numSpills);
            }
            saveKvOffsets(true);
            sortAndSpill();
          }
      ...
      // shut down spill thread and wait for it to exit.
    ...
    private final class SpillThread extends Thread {
    ...
    private void sortAndSpill() throws IOException, ClassNotFoundException,
      ...
      try {
        ...
        out = useMapRFs ?
          rfs.createFid(mapOutputFile.getSpillFid(),
            mapOutputFile.getSpillFileForWriteFid(getTaskID(), numSpills,
              size)) :
          rfs.create(mapOutputFile.getSpillFileForWrite(getTaskID(), numSpills,
            size));
      ...
      for ( int k = 0; k < threadsNumber; k++ ) {
        ...
        public Integer call() throws Exception {
          ...
          for (int i = maxLoop*someOff ; i < endValue; ++i) {
            ...
            FSDataOutputStream finalOut = useMapRFs ?
              rfs.createFid(mapOutputFile.getOutputFid(),
                mapOutputFile.getOutputFileForWriteFid(mapId, -1, i)) :
              rfs.create(mapOutputFile.getOutputFileForWrite(mapId, -1, i));
    ...
    protected class MRResultIterator implements RawKeyValueIterator {
      ...
      private int current = 0, end = 0;
      private KvOffset offsets;

      public MRResultIterator(KvOffset ko) {
        this.offsets = ko;
        this.current = -1;
        this.end  = ko.size();
      }
    ...
    /* Merge spill files and create an output file per partition 
     * Used in maprfs.
     * */
    private void mergePerPartition() throws IOException, InterruptedException,
	         ClassNotFoundException {
      final Path[] filename = new Path[numSpills];
      final FSDataInputStream[] filein = new FSDataInputStream[numSpills];
      final TaskAttemptID mapId = getTaskID();
      ...
      for (int parts = 0; parts < partitions; parts++) {
        ...
        FSDataOutputStream finalOut = rfs.createFid(
          ...
          mapOutputFile.getOutputFileForWriteFid(mapId, -1, parts));
        ...
        for (int i = 0; i < numSpills; i++) {
          ...
          Segment<K,V> s = new Segment<K,V>(job, rfs, filename[i], 
                 indexRecord.startOffset, indexRecord.partLength, codec, true, filein[i]);
          segmentList.add(i, s);
          if (LOG.isDebugEnabled()) {
            LOG.debug("MapId=" + mapId + " Reducer=" + parts +
              "Spill =" + i + "(" + indexRecord.startOffset + "," +
