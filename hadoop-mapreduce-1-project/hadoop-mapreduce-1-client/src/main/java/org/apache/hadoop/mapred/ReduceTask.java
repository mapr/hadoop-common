...
class ReduceTask extends Task {
  ...
  private boolean useDirectReduce;
  ...
  /* MapR */
  public Path getTmpDir(String id) {
    return (useMapRFs)? 
      mapOutputFile.getLocalPathForWrite(id, -1) : new Path(id);
  }
  ...
  public void run(JobConf job, final TaskUmbilicalProtocol umbilical)
    ...
    this.prefetchBytesHint = conf.getInt("maprfs.openfid2.prefetch.bytes", -1);

    
    rfs  = (useMapRFs)? FileSystem.get(job): FileSystem.getLocal(job).getRaw();
    useDirectReduce = job.getBoolean("mapreduce.use.fastreduce", false);
    if (useDirectReduce) 
      LOG.warn("Using fast reduce with io.sort.factor= " + 
                Math.max(2, job.getInt("io.sort.factor", -1)));
    if (isMapOrReduce()) {
    ...
    boolean isLocal = "local".equals(job.get("mapred.job.tracker"));
    ...
    RawKeyValueIterator rIter = isLocal
      ? Merger.merge(job, rfs, job.getMapOutputKeyClass(),
          ...
          Math.max(2, job.getInt("io.sort.factor", -1)),
          getTmpDir(getTaskID().toString()), job.getOutputKeyComparator(),
  ...
  class ReduceCopier<K, V> implements MRConstants {
     ...
     */
    private LinkedList<MapOutputLocation> scheduledCopiesTail =
      new LinkedList<MapOutputLocation>();

    private final Object scheduledCopiesTailLock = new Object();

    private LinkedList<MapOutputLocation> scheduledCopiesHead =
      new LinkedList<MapOutputLocation>();

    private final Object scheduledCopiesHeadLock = new Object();

    /**
     *  the results of dispatched copy attempts
     */
    private LinkedList<CopyResult> copyResults =
      new LinkedList<CopyResult>();

    private final Object copyResultsLock = new Object();
     ...
     */
    private LinkedList<MapOutputLocation> retryFetches =
      new LinkedList<MapOutputLocation>();
    
    /** 
     * The set of required map outputs
     */
    private final Map<TaskID,String> copiedMapOutputs =
      new ConcurrentHashMap<TaskID,String>();
    private static final String DUMMY_STRING = "DUMMY_STRING";

    /** 
     * The set of obsolete map taskids.
     */
    private Map<TaskAttemptID,String> obsoleteMapIds =
      new ConcurrentHashMap<TaskAttemptID,String>();
     ...
     */
    Set<TaskID> fetchFailedMaps = new HashSet<TaskID>(); 
     ...
     */
    private final ConcurrentLinkedQueue<MapOutput> mapOutputsFilesInMemory =
      new ConcurrentLinkedQueue<MapOutput>();
...
*/
    ...
    public ReduceCopier(TaskUmbilicalProtocol umbilical, JobConf conf,
      ...
      this.numCopiers = conf.getInt("mapred.reduce.parallel.copies", -1);
      ...
      this.ioSortFactor =  Math.max(2, conf.getInt("io.sort.factor", -1));
      ...
      if (useMapRFs) {
        this.rfs = FileSystem.get(conf);
        this.localFileSys = rfs;
      } else {
        this.localFileSys = FileSystem.getLocal(conf);
        this.rfs = ((LocalFileSystem)localFileSys).getRaw();
      }
    ...
    void scheduleNewMaps(LinkedList<MapOutputLocation> locs) {
      final int numScheduled = locs.size();
      ListIterator<MapOutputLocation> it =
        locs.listIterator(random.nextInt(numScheduled));
      ...
      for (boolean wrapped = false; it.hasNext();) {
        ...
        if (!wrapped && !it.hasNext()) {
          wrapped = true;
          it = locs.listIterator(0);
        }

        // Do not schedule fetches from OBSOLETE maps
        if (obsoleteMapIds.containsKey(loc.getTaskAttemptId())
         || copiedMapOutputs.containsKey(loc.getTaskId())) {
          continue;
        }

        synchronized (scheduledCopiesTailLock) {
          scheduledCopiesTail.add(loc);
          scheduledCopiesTailLock.notify();
        }
      }

      if (LOG.isInfoEnabled()) {
        LOG.info("scheduleNewMaps numScheduled " + numScheduled + " numCopied " + numCopied);
      }
    }
    ...
    private long createInMemorySegments(
      ...
      synchronized (mapOutputsFilesInMemory) {
        ...
        while(fullSize > leaveBytes) {
          MapOutput mo = mapOutputsFilesInMemory.remove();
    ...
    private RawKeyValueIterator createKVIterator(
      ...
      final Path tmpDir = getTmpDir(getTaskID().toString());
      ...
      final MapOutput mapOutput = mapOutputsFilesInMemory.peek();
      if (mapOutput != null) {
        TaskID mapId = mapOutput.mapId;
      ...
      if (useDirectReduce) {
        for (FileStatus filestatus : mapOutputFilesOnDisk) {
          ...
          diskSegments.add(new Segment<K, V>(job, fs, filestatus.getPath(), codec, keepInputs));
        }
    ...
    class RawKVIteratorReader extends IFile.Reader<K,V> {
      ...
      // closeInStream is ignored for RawKVIteratorReader
      public void close(boolean closeInStream) throws IOException {
        kvIter.close();
      }
    }

    private LinkedList<CopyResult> getCopyResults() {
      synchronized (copyResultsLock) {
        while (copyResults.isEmpty()) {
          try {
            copyResultsLock.wait();
          } catch (InterruptedException e) { }
        }

        final LinkedList<CopyResult> res = copyResults;
        copyResults = new LinkedList<CopyResult>();
        return res;
      }
    }
    ...
    private class LocalFSMerger extends Thread {
      ...
      public LocalFSMerger(FileSystem fs) {
      ...
      public void run() {
        try {
          ...
          while(!exitLocalFSMerge){
            ...
            Path outputPath;
            if (useMapRFs) {
              // get a file from maprfs temp storage
              outputPath = mapOutputFile.getLocalPathForWrite(
                  mapFiles.get(0).toString(),
                  approxOutputSize).suffix(".merged");
            } else {
              // add the checksum length
              int bytesPerSum = 
                reduceTask.getConf().getInt("io.bytes.per.checksum", 512);
              approxOutputSize += ChecksumFileSystem
                .getChecksumLength(approxOutputSize, bytesPerSum);
              outputPath = 
            ...
            Path tmpDir = getTmpDir(reduceTask.getTaskID().toString());	    
            try {
    ...
    private class InMemFSMergeThread extends Thread {
      ...
      private void doInMemMerge() throws IOException{
        final MapOutput mo = mapOutputsFilesInMemory.peek();
        if (mo == null) {
          return;
        }
        ...
        try {
          ...
          rIter = Merger.merge(conf, rfs,
                               ...
                               getTmpDir(reduceTask.getTaskID().toString()),
                               conf.getOutputKeyComparator(), reporter,
                               spilledRecordsCounter, null);
    ...
    private class GetMapEventsThread extends Thread {
      ...
      private final IntWritable fromEventId = new IntWritable(0);
      ...
      public void run() {
        ...
        do {
          try {
            ...
            // MapR: MAPREDUCE-215,7 do not return on InterruptedException
            if (exitGetMapEvents) {
              LOG.warn(reduceTask.getTaskID() +
              ...
              return;
               ...
               LOG.warn ("GetMapEventsThread Unexpected InterruptedException");
             }
          }
          catch (Throwable t) {
        ...
        for (TaskCompletionEvent event : events) {
          ...
          switch (event.getTaskStatus()) {
            case SUCCEEDED:
            {
              ...
              newMapOutputs.add(
                new MapOutputLocation(taskId, host, event.shuffleRootFid));
              numNewMaps++;
            }
            break;
            case FAILED:
            ...
            case OBSOLETE:
            {
              obsoleteMapIds.put(event.getTaskAttemptId(), DUMMY_STRING);
