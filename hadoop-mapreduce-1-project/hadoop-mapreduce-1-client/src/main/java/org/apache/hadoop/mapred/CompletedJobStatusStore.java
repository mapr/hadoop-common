...
class CompletedJobStatusStore implements Runnable {
  ...
  CompletedJobStatusStore(Configuration conf, ACLsManager aclsManager)
    ...
    if (active) {
      ...
      jobInfoDir = conf.get("mapred.job.tracker.persist.jobstatus.dir");
  ...
  public void store(JobInProgress job) {
    if (active && retainTime > 0) {
      ...
      try {
        ...
        TaskCompletionEventList events =
                job.getTaskCompletionEventList(0, Integer.MAX_VALUE);
        events.write(dataOut);
        dataOut.close();
  ...
  private TaskCompletionEventList readEvents(FSDataInputStream dataIn,
                                           int offset, int len)
          throws IOException {
    ...
    return new TaskCompletionEventList(events, 0, events.length);
  }
   ...
   */
  public TaskCompletionEventList readJobTaskCompletionEvents(JobID jobId,
                                                               int fromEventId,
                                                               int maxEvents) {
    TaskCompletionEventList events = TaskCompletionEventList.EMPTY;
    if (active) {
      try {
        FSDataInputStream dataIn = getJobInfoFile(jobId);
        if (dataIn != null) {
