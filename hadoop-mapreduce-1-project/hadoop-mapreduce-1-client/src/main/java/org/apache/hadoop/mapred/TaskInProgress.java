...
public class TaskInProgress {
  static enum KillMode { USER_FAIL, USER_KILL, SCHEDULER_PREEMPT };
  ...
  private MetricsRecord maprTaskMetricFinal;
  private static MetricsContext maprMetricsContextTimeVar = MetricsUtil.getContext("maprmepredvariant");
  ...
  private TIPStatus prevTaskStatus;
  ...
  private Map<TaskAttemptID, MetricRecordWithBits> taskAttemptFinalRecords = new HashMap<TaskAttemptID, MetricRecordWithBits>();
  private Map<TaskAttemptID, MetricsRecord> taskAttemptTimeVarRecords = new HashMap<TaskAttemptID, MetricsRecord>();
  private TaskInProgressMetricsUpdater taskInProgressUpdater;
  /**
  ...
  private TreeMap<TaskAttemptID,KillMode> tasksToKill =
    new TreeMap<TaskAttemptID, KillMode>();
  ...
  void recordTaskMetricStart() {
    String splits = getSplitNodes();
    if ( !splits.isEmpty()) {
      this.maprTaskMetricFinal.setTag("f_splitlocations", getSplitNodes());
    }
    this.maprTaskMetricFinal.setTag("f_starttime", getExecStartTime());
    this.maprTaskMetricFinal.update();
    this.maprTaskMetricFinal.removeTag("f_splitlocations");
  }
  
  void recordTaskMetricFinish(TIPStatus status) {
    this.maprTaskMetricFinal.remove();
    this.maprTaskMetricFinal.setTag("f_finishtime", getExecFinishTime());
    this.maprTaskMetricFinal.setTag("f_status", status.toString());
    this.maprTaskMetricFinal.update();
    clearAllTaskAttemptMetrics();
  }
  /**
  ...
  public void setJobCleanupTask() {
    jobCleanup = true;
    this.maprTaskMetricFinal.setTag("f_type", TaskType.JOB_CLEANUP.name());
  }
  ...
  public void setJobSetupTask() {
    jobSetup = true;
    this.maprTaskMetricFinal.setTag("f_type", TaskType.JOB_SETUP.name());
  }
  ...
  void init(JobID jobId) {
    ...
    this.maprTaskMetricFinal = MetricsUtil.createRecord(maprMetricsContext, "task_final"); 
    this.maprTaskMetricFinal.setTag("f_id", this.id.toString());
    this.maprTaskMetricFinal.setTag("f_jobid", jobId.toString());
    this.maprTaskMetricFinal.setTag("f_partitionid", job.getStartTime());
    if ( isMapTask() ) {
      this.maprTaskMetricFinal.setTag("f_type", TaskType.MAP.name());
    } else {
      this.maprTaskMetricFinal.setTag("f_type", TaskType.REDUCE.name());
    }
    taskInProgressUpdater = new TaskInProgressMetricsUpdater();
    maprMetricsContextTimeVar.registerUpdater(taskInProgressUpdater);
  ...
  public void setExecFinishTime(long finishTime) {
    ...
    this.maprTaskMetricFinal.setTag("f_finishtime", execFinishTime);
    this.maprTaskMetricFinal.update();
    this.maprTaskMetricFinal.remove();
    clearAllTaskAttemptMetrics();
  }
  ...
  private void setSuccessfulTaskid(TaskAttemptID successfulTaskId) {
    ...
    this.maprTaskMetricFinal.setTag("f_staskattemptid", successfulTaskId.toString());
    this.maprTaskMetricFinal.setTag("f_finishtime", jobtracker.getClock().getTime());
    this.maprTaskMetricFinal.setTag("f_status", TIPStatus.COMPLETE.toString());
    this.maprTaskMetricFinal.update();
  }
  ...
  private void resetSuccessfulTaskid() {
    ...
    this.maprTaskMetricFinal.remove();
    // we may lost this record already
    // need to recreate it
    this.maprTaskMetricFinal = MetricsUtil.createRecord(maprMetricsContext, "task_final"); 
    this.maprTaskMetricFinal.setTag("f_id", this.id.toString());
    this.maprTaskMetricFinal.setTag("f_jobid", this.id.getJobID().toString());
    this.maprTaskMetricFinal.setTag("f_partitionid", job.getStartTime());
    if ( isMapTask() ) {
      this.maprTaskMetricFinal.setTag("f_type", TaskType.MAP.name());
    } else {
    ...
    this.maprTaskMetricFinal.setTag("f_staskattemptid", "");
    this.maprTaskMetricFinal.update();
    }
  ...
  /**
   * MapR:Bug 4025 Should the task cleanup be launched.
   */
  public boolean shouldScheduleCleanup(TaskAttemptID taskid) {
    TaskStatus.State state = getTaskStatus(taskid).getRunState();
    // MapR:Bug 4025 Schedule only if no task cleanup attempt is yet reported.
...
/*    MetricsRecord maprTaskAtMetricEvent = taskAttemptEventRecords.get(taskId);
    if ( maprTaskAtMetricEvent != null ) {
      //maprTaskAtMetricEvent.remove();
      maprTaskAtMetricEvent.setTag("e_diagnosticinfo", diagInfo);
      maprTaskAtMetricEvent.update();
      maprTaskAtMetricEvent.removeTag("e_diagnosticinfo");
    }
*/  MetricRecordWithBits metricFRecord = taskAttemptFinalRecords.get(taskId);
    if ( metricFRecord == null ) {
      MetricsRecord tAtFinalRecord = MetricsUtil.createRecord(maprMetricsContext, "taskattempt_final"); 
      tAtFinalRecord.setTag("f_id", taskId.toString());
      tAtFinalRecord.setTag("f_taskid", taskId.getTaskID().toString());
      tAtFinalRecord.setTag("f_jobid", taskId.getJobID().toString());
      tAtFinalRecord.setTag("f_partitionid", job.getStartTime());
      taskAttemptFinalRecords.put(taskId, new MetricRecordWithBits(tAtFinalRecord));
      metricFRecord = taskAttemptFinalRecords.get(taskId);
    }
    
    MetricsRecord maprTaskAtMetricFinal = metricFRecord.getMetricsRecord();
  	if ( maprTaskAtMetricFinal != null ) {
  	  maprTaskAtMetricFinal.setTag("f_diagnosticinfo", StringUtils.arrayToString(diagHistory.toArray(new String[0])));
  	  maprTaskAtMetricFinal.update();
  	  maprTaskAtMetricFinal.removeTag("f_diagnosticinfo");
  	}
  	
  	st.setDiagnosticInfo(diagInfo);
  }
  
  private void metricsUpdate(TaskStatus status, boolean changed) {
    TaskAttemptID taskid = status.getTaskID();

    MetricRecordWithBits metricFRecord = taskAttemptFinalRecords.get(taskid);
    
    if ( metricFRecord != null ) {
      boolean isNeedMUpdate = false;
      if ( !metricFRecord.isFieldSet(MetricRecordWithBits.Fields.LOG_LOCATION)) {
        TaskTrackerStatus ttStatus = jobtracker.getTaskTrackerStatus(status.getTaskTracker());
        if ( ttStatus != null && ttStatus.getHost() != null && status.getTaskLogFs() != null ) {
          String logLocation = TaskLogServlet.getTaskLogUrl(ttStatus.getHost(), String.valueOf(ttStatus.getHttpPort()), status.getTaskID().toString());
          logLocation = logLocation + "&tlogfs=" + status.getTaskLogFs().name();
          metricFRecord.getMetricsRecord().remove();
          metricFRecord.getMetricsRecord().setTag("f_log", logLocation);
          if ( status.getStartTime() > 0 )
            metricFRecord.getMetricsRecord().setTag("f_starttime", status.getStartTime());
          metricFRecord.setBit(MetricRecordWithBits.Fields.LOG_LOCATION);
          
          isNeedMUpdate = true;
        }
      }
      if ( changed && metricFRecord.getMetricsRecord() != null) {
        metricFRecord.getMetricsRecord().setTag("f_state", status.getRunState().name());
        TIPStatus taskStatus = getTaskStatus();
        if ( prevTaskStatus == null || prevTaskStatus != taskStatus ) {
          prevTaskStatus = taskStatus;
          if ( taskStatus != null && taskStatus != TIPStatus.COMPLETE
              && taskStatus != TIPStatus.KILLED && taskStatus != TIPStatus.FAILED ) {
            this.maprTaskMetricFinal.setTag("f_status", taskStatus.toString());
            this.maprTaskMetricFinal.update();
            this.maprTaskMetricFinal.removeTag("f_status");
          }
        }
        isNeedMUpdate = true;
      }      
      if ( !metricFRecord.isFieldSet(MetricRecordWithBits.Fields.START_TIME)) {
        if ( status.getStartTime() > 0 ) {
          if ( !isNeedMUpdate ) {
            metricFRecord.getMetricsRecord().remove();
          }
          metricFRecord.getMetricsRecord().setTag("f_starttime", status.getStartTime());
          metricFRecord.setBit(MetricRecordWithBits.Fields.START_TIME);
          isNeedMUpdate = true;
        }
      }
      if ( !status.getIsMap()) {
        if ( status.getShuffleFinishTime() > 0 &&
            !metricFRecord.isFieldSet(MetricRecordWithBits.Fields.SHUFFLE_FINISH)) {
          if ( !isNeedMUpdate ) {
            metricFRecord.getMetricsRecord().remove();
          }
          metricFRecord.setBit(MetricRecordWithBits.Fields.SHUFFLE_FINISH);
          metricFRecord.getMetricsRecord().setTag("f_shufflefinishtime", status.getShuffleFinishTime());
          if ( status.getStartTime() > 0 )
            metricFRecord.getMetricsRecord().setTag("f_starttime", status.getStartTime());
          isNeedMUpdate = true;
        }
        if ( status.getSortFinishTime() > 0 &&
            !metricFRecord.isFieldSet(MetricRecordWithBits.Fields.SORT_FINISH) ) {
          if ( !isNeedMUpdate ) {
            metricFRecord.getMetricsRecord().remove();
          }
          metricFRecord.setBit(MetricRecordWithBits.Fields.SORT_FINISH);
          metricFRecord.getMetricsRecord().setTag("f_sortfinishtime", status.getSortFinishTime());
          if ( status.getStartTime() > 0 )
            metricFRecord.getMetricsRecord().setTag("f_starttime", status.getStartTime());
          isNeedMUpdate = true;
        }
      ...
      if ( isNeedMUpdate ) {
        metricFRecord.getMetricsRecord().update();
      }
      /*if ( status.getFinishTime() > 0 ) {
        metricFRecord.getMetricsRecord().remove();
        metricFRecord.setBit(MetricRecordWithBits.Fields.FINISH_TIME);
        metricFRecord.getMetricsRecord().setTag("f_finishtime", status.getFinishTime());
        if ( status.getStartTime() > 0 )
          metricFRecord.getMetricsRecord().setTag("f_starttime", status.getStartTime());
        metricFRecord.getMetricsRecord().update();
      } else if ( isNeedMUpdate ) {
        metricFRecord.getMetricsRecord().update();
      } */
    }
  }
  ...
  synchronized boolean updateStatus(TaskStatus status) {
    ...
    if (diagInfo != null && diagInfo.length() > 0) {
      LOG.info("Error from "+taskid+": "+diagInfo + 
               " on tasktracker " + status.getTaskTracker());
      addDiagnosticInfo(taskid, diagInfo, status);
    }
    ...
    if (oldStatus != null) {
      ...
      if (oldState == TaskStatus.State.FAILED ||
          oldState == TaskStatus.State.KILLED) {
        tasksToKill.put(taskid, KillMode.USER_FAIL);
        return false;	  
      }

/*      MetricRecordWithBits metricFinalWBits =  taskAttemptFinalRecords.get(taskid);
      if ( metricFinalWBits != null ) {
        MetricsRecord metricFinalRecord = metricFinalWBits.getMetricsRecord();
        if ( metricFinalRecord != null ) {
          if ( oldStatus == null || oldState != newState) {
            metricFinalRecord.setTag("f_state", status.getRunState().name());
            metricFinalRecord.update();
            metricFinalRecord.removeTag("f_state");
            TIPStatus taskStatus = getTaskStatus();
            if ( taskStatus != null ) {
              this.maprTaskMetricFinal.setTag("f_status", taskStatus.toString());
              this.maprTaskMetricFinal.update();
...
*/      changed = oldState != newState;
    ...
    if (!isCleanupAttempt(taskid)) {
      if ( oldStatus != null && status.getCounters().size() <  oldStatus.getCounters().size()) {
       status.setCounters(oldStatus.getCounters()); 
      }
  ...
  public void incompleteSubTask(TaskAttemptID taskid, 
    ...
    if (status != null) {
      ...
      final KillMode killMode = tasksToKill.remove(taskid);
      final boolean shouldFail = (killMode == KillMode.USER_FAIL);
      if (shouldFail) {
        ...
        if (killMode == KillMode.SCHEDULER_PREEMPT) {
          String diagInfo = "Task has been " + taskState
                   + " by TaskScheduler for preemption.";
          addDiagnosticInfo(taskid, diagInfo, status);          
        } else {
          String diagInfo = "Task has been " + taskState + " by the user";
          addDiagnosticInfo(taskid, diagInfo, status);
        }
      }
      ...
      if (0 == status.getFinishTime() && status.getStartTime() > 0){
    ...
    finishMetrics(taskid);
    
    if (numTaskFailures >= maxTaskAttempts) {
      LOG.info("TaskInProgress " + getTIPId() + " has failed " + numTaskFailures + " times.");
      kill(TIPStatus.FAILED, null);
    }
    this.user = job.getUser();
  }
  
  private void finishMetrics(TaskAttemptID taskId) {
    // need to make sure then after we remove those records they don't pop up again
    synchronized(taskAttemptTimeVarRecords) {
      MetricsRecord timeVarRecord = taskAttemptTimeVarRecords.get(taskId);
      if ( timeVarRecord !=  null ) {
        timeVarRecord.remove();
        taskAttemptTimeVarRecords.remove(taskId);
      }
    }
    TaskStatus status = taskStatuses.get(taskId);
    MetricRecordWithBits mrWB = taskAttemptFinalRecords.get(taskId);
    if ( mrWB != null ) {
      MetricsRecord metricFRecord = mrWB.getMetricsRecord();
      if ( metricFRecord != null ) {
        metricFRecord.remove();
        metricFRecord.setTag("f_finishtime", status.getFinishTime());
        metricFRecord.setTag("f_state", status.getRunState().name());
        metricFRecord.setMetric("f_PROGRESS", 100l);
        for (Counters.Group group : status.getCounters()) {
          for (Counters.Counter counter : group) {
            metricFRecord.setMetric("f_"+counter.getName(), counter.getCounter());
          }
        }
        metricFRecord.update();
      }
    }
  }
  /**
  ...
  void alreadyCompletedTask(TaskAttemptID taskid) {
    ...
    addDiagnosticInfo(taskid, "Already completed TIP", getTaskStatus(taskid));
  ...
  private void clearAllTaskAttemptMetrics() {
    maprMetricsContextTimeVar.unregisterUpdater(taskInProgressUpdater);
    synchronized(taskAttemptTimeVarRecords) {
      for ( MetricsRecord record : taskAttemptTimeVarRecords.values() ) {
        record.removeTag("ma_hostid");
        record.removeTag("m_partitionid");
        record.remove();
      }
      taskAttemptTimeVarRecords.clear();
    }
    for ( MetricRecordWithBits record : taskAttemptFinalRecords.values() ) {
      record.getMetricsRecord().removeTag("f_sortfinishtime");
      record.getMetricsRecord().removeTag("f_diagnosticinfo");
      record.getMetricsRecord().removeTag("f_starttime");
      record.getMetricsRecord().removeTag("f_shufflefinishtime");
      record.getMetricsRecord().removeTag("f_finishtime");
      record.getMetricsRecord().removeTag("f_type");
      record.getMetricsRecord().removeTag("f_state");
      record.getMetricsRecord().removeTag("f_log");
      record.getMetricsRecord().removeTag("f_host");
      record.getMetricsRecord().removeTag("f_partitionid");
      record.getMetricsRecord().remove();
    }
    taskAttemptFinalRecords.clear();
  }
  
  public void clearTaskAndTaskAttemptFinalMetrics() {
    for ( MetricRecordWithBits record : taskAttemptFinalRecords.values() ) {
      record.getMetricsRecord().removeTag("f_sortfinishtime");
      record.getMetricsRecord().removeTag("f_diagnosticinfo");
      record.getMetricsRecord().removeTag("f_starttime");
    ...
    this.maprTaskMetricFinal.removeTag("f_status");
    this.maprTaskMetricFinal.removeTag("f_finishtime");
    this.maprTaskMetricFinal.removeTag("f_starttime");
    this.maprTaskMetricFinal.removeTag("f_splitlocations");
    this.maprTaskMetricFinal.removeTag("f_type");
    this.maprTaskMetricFinal.removeTag("f_staskattemptid");
    this.maprTaskMetricFinal.removeTag("f_partitionid");
    this.maprTaskMetricFinal.remove();
  }
  ...
  public long getCompletedAttemptTime() {
    if (isComplete()) {
      TaskStatus[] statuses = getTaskStatuses();
      for(int i=0; i < statuses.length; i++) {
        if (statuses[i].getRunState() == TaskStatus.State.SUCCEEDED) {
          return statuses[i].getFinishTime() - statuses[i].getStartTime();
        }
      }
    }
    return 0;
  ...
  private void kill(TIPStatus status, String reason) {
    ...
    killed = true;
    this.execFinishTime = jobtracker.getClock().getTime();
    if ( !isJobCleanupTask() && !isJobSetupTask() ) {
      this.maprTaskMetricFinal.remove();
      // if exec time is not set - this task was not inited, don't bother
      if ( getExecStartTime() != 0 ) {
        this.maprTaskMetricFinal.setTag("f_finishtime", getExecFinishTime());
        this.maprTaskMetricFinal.setTag("f_status", status.toString());
        this.maprTaskMetricFinal.update();
      }
    ...
    if (reason != null && !reason.isEmpty()) {
      for (Iterator<TaskAttemptID> it = taskStatuses.keySet().iterator(); it.hasNext();) {
         ...
         taskStatus.setDiagnosticInfo(reason);
         LOG.warn("Killing task " + taskid + ", reason: " + reason);
      }
    }
   ...
   */
  public void kill() {
    kill(TIPStatus.KILLED, null);
  }

  public void kill(String reason) {
    kill(TIPStatus.KILLED, reason);
  }
   ...
   * Kill the given task
   */
  boolean killTask(TaskAttemptID taskId, KillMode killMode) {
    ...
    if(st != null && (st.getRunState() == TaskStatus.State.RUNNING
        ...
        && tasksToKill.put(taskId, killMode) == null ) {
      final String logStr;
      if (killMode != KillMode.SCHEDULER_PREEMPT) {
        logStr =   "Request received to " + killMode + " task '" + taskId
                 + "' by user";
      } else {
        logStr = "TaskScheduler requests to preempt task " + taskId;
      }
      addDiagnosticInfo(taskId, logStr, st);
  ...
  public Task addRunningTask(TaskAttemptID taskid, 
    ...
    tasks.add(taskid);
    String hostName = JobInProgress.convertTrackerNameToHostName(taskTracker);
    MetricsRecord tAtFinalRecord = MetricsUtil.createRecord(maprMetricsContext, "taskattempt_final"); 
    tAtFinalRecord.setTag("f_id", taskid.toString());
    tAtFinalRecord.setTag("f_taskid", taskid.getTaskID().toString());
    tAtFinalRecord.setTag("f_jobid", taskid.getJobID().toString());
    if ( taskType != null ) {
      tAtFinalRecord.setTag("f_type", taskType.name());
    }
    tAtFinalRecord.setTag("f_host", hostName);
    tAtFinalRecord.setTag("f_partitionid", job.getStartTime());
    taskAttemptFinalRecords.put(taskid, new MetricRecordWithBits(tAtFinalRecord));

    MetricsRecord tAtTVRecord = MetricsUtil.createRecord(maprMetricsContextTimeVar, "taskattempt_variant"); 
    tAtTVRecord.setTag("ma_id", taskid.toString());
    tAtTVRecord.setTag("ma_taskid", taskid.getTaskID().toString());
    tAtTVRecord.setTag("ma_jobid", taskid.getJobID().toString());
    tAtTVRecord.setTag("ma_hostid", hostName);
    tAtTVRecord.setTag("m_partitionid", job.getStartTime());
    taskAttemptTimeVarRecords.put(taskid, tAtTVRecord);
  ...
  private void updateMetrics() {
    for ( Map.Entry<TaskAttemptID, TaskStatus> statusEntry : taskStatuses.entrySet()) {
      TaskAttemptID taskid = statusEntry.getKey();
      TaskStatus status = statusEntry.getValue();
      Counters copyCounters = new Counters();
      copyCounters.incrAllCounters(status.getCounters());
      // make sure we don't update after record is removed
      synchronized(taskAttemptTimeVarRecords) {
        MetricsRecord metricRecord = taskAttemptTimeVarRecords.get(taskid);
        if ( metricRecord != null ) {
          metricRecord.setMetric("m_PROGRESS", (long) (status.getProgress()*100));
          for (Counters.Group group : copyCounters) {
            for (Counters.Counter counter : group) {
              metricRecord.setMetric("m_"+counter.getName(), counter.getCounter());
            }
          }
          metricRecord.update();
        }
  ...
  private class TaskInProgressMetricsUpdater implements Updater {

    public TaskInProgressMetricsUpdater() {
    }
  ...
  private static class MetricRecordWithBits {
    public static enum Fields {
      SORT_FINISH("f_sortfinishtime", 1),
      START_TIME("f_starttime",2),
      SHUFFLE_FINISH("f_shufflefinishtime",3),
      FINISH_TIME("f_finishtime",4),
      TASKATT_TYPE("f_type", 5),
      TASKATT_STATE("f_state",6),
      LOG_LOCATION("f_log",7);
      
      private String metricName;
      private int bit;
      private Fields(String metricName, int bit) {
    ...
    private BitSet recordBitMask = new BitSet();
    
    public MetricRecordWithBits(MetricsRecord record) {
      this.record = record;
    }
    
    public void setBit(Fields field) {
      recordBitMask.set(field.getBit());
      return;
    }
    
    public boolean isFieldSet(Fields field) {
      return recordBitMask.get(field.getBit());
    }
