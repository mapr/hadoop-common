...
public abstract class CombineFileInputFormat<K, V>
  ...
  private static final Log LOG
    = LogFactory.getLog(CombineFileInputFormat.class);
  ...
  private void getMoreSplits(Configuration conf, Path[] paths, 
    ...
    for (Iterator<Map.Entry<String, 
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("Combining split by node: " + one.getKey());
      }
      ...
      for (OneBlockInfo oneblock : blocksInNode) {
        if (blockToNodes.containsKey(oneblock)) {
          ...
          if (maxSize != 0 && curSplitSize >= maxSize) {
            ...
            if (LOG.isDebugEnabled()) {
              LOG.debug("Combining node-splits on "
                + one.getKey() + " because maxSplitSize="
                + maxSize + " is reached. Listing blocks:");
              for (OneBlockInfo obi : validBlocks) {
                LOG.debug(obi);
              }
            }
            addCreatedSplit(conf, splits, nodes, validBlocks);
            curSplitSize = 0;
            validBlocks.clear();
          }
        }
      ...
      if (minSizeNode != 0 && curSplitSize >= minSizeNode) {
        if (LOG.isDebugEnabled()) {
          LOG.debug("Combining node-splits on "
            + one.getKey() + " because minSizeNode="
            + minSizeNode + " is reached. Listing blocks:");
          for (OneBlockInfo obi : validBlocks) {
            LOG.debug(obi);
          }
        }
      ...
      for (Iterator<Map.Entry<String, List<OneBlockInfo>>> iter = 
        ...
        for (OneBlockInfo oneblock : blocks) {
          if (blockToNodes.containsKey(oneblock)) {
            ...
            if (maxSize != 0 && curSplitSize >= maxSize) {
              if (LOG.isDebugEnabled()) {
                LOG.debug("Combining rack-splits on "
                  + one.getKey() + " because maxSplitSize="
                  + maxSize + " is reached. Listing blocks:");
                for (OneBlockInfo obi : validBlocks) {
                  LOG.debug(obi);
              ...
              // create an input split and add it to the splits array
              addCreatedSplit(conf, splits, getHosts(racks), validBlocks);
              createdSplit = true;
              break;
            }
          }
        ...
        if (!validBlocks.isEmpty()) {
          if (minSizeRack != 0 && curSplitSize >= minSizeRack) {
            if (LOG.isDebugEnabled()) {
              LOG.debug("Combining rack-splits on "
                + one.getKey() + " because minSizeRack="
                + minSizeRack + " is reached. Listing blocks:");
              for (OneBlockInfo obi : validBlocks) {
                LOG.debug(obi);
              }
            }
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("Combining overflow splits");
    }
    for (OneBlockInfo oneblock : overflowBlocks) {
      ...
      if (maxSize != 0 && curSplitSize >= maxSize) {
        if (LOG.isDebugEnabled()) {
          LOG.debug("Combining overflow because maxSplitSize="
            + maxSize + " is reached. Listing blocks:");
          for (OneBlockInfo obi : validBlocks) {
            LOG.debug(obi);
          }
    ...
    if (!validBlocks.isEmpty()) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("Combining remaining overflow blocks. Listing blocks:");
        for (OneBlockInfo obi : validBlocks) {
          LOG.debug(obi);
        }
      }
   ...
   */
  private void addCreatedSplit(Configuration conf,
                               List<InputSplit> splitList, 
                               List<String> locations, 
                               ArrayList<OneBlockInfo> validBlocks) {
    final HashMap<String,HostCounter> hostMetric =
    ...
    for (int i = 0; i < validBlocks.size(); i++) {
      ...
      for (String h : oneblock.hosts) {
        HostCounter hc = hostMetric.get(h);
        if (hc == null) {
          hc = new HostCounter(h);
          hostMetric.put(h, hc);
        }
        hc.val += oneblock.length;
      }
    }
    final int k = conf.getInt("mapred.multi.split.locations", 3);
    if (LOG.isDebugEnabled()) {
      LOG.debug("Determine top-" + k + " bytes locations for split");
    }
    final String[] topKArray;
    final Set<String> hostKeySet = hostMetric.keySet();
    ...
    if (k < hostKeySetSize) {
      ...
      for (it = hostMetric.values().iterator(); it.hasNext(); ) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("k=" + k + " >= keySetSize=" + hostKeySetSize
          + ": using all locations");
        for (HostCounter hc : hostMetric.values()) {
          LOG.debug("Location: " + hc);
        }
      }
      topKArray = new String[hostMetric.size()];
