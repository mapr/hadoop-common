...
class Merger {  
  ...
  public static class Segment<K extends Object, V extends Object> {
    ...
    public Segment(Configuration conf, FileSystem fs, Path file,
            long segmentOffset, long segmentLength, CompressionCodec codec,
            boolean preserve, FSDataInputStream in) throws IOException {
      this.conf = conf;
      this.fs = fs;
      this.file = file;
      this.codec = codec;
    ...
    private void init(Counters.Counter readsCounter) throws IOException {
        if (reader == null) {
          FSDataInputStream in = (fin != null) ? fin : fs.open(file);
          in.seek(segmentOffset);
          // fin == null implies the instream will be closed/re-opened.
          reader = new Reader<K, V>(conf, in, segmentLength, codec, readsCounter, fin == null);          
        }
    }
    ...
    void close() throws IOException {
      reader.close((fin == null) ? true : false);
  ...
  extends PriorityQueue<Segment<K, V>> implements RawKeyValueIterator {
    ...
    RawKeyValueIterator merge(Class<K> keyClass, Class<V> valueClass,
	  ...
	  Path outputFile = (TaskTracker.useMapRFs(conf))? tmpFilename :
