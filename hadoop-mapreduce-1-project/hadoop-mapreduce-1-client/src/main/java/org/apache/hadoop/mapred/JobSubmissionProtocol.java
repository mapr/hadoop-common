...
interface JobSubmissionProtocol extends VersionedProtocol {
  ...
  /**
   * Ask job to unblacklist a node and start scheduling on it (expert only)
   */
  public int unBlackListTracker(JobID jobid, String hostname) throws IOException;

  /* blacklist a tasktracker for entire cluster (admin only) */
  public int blackListTaskTracker(String hostname) throws IOException;
  
  /* unblacklist a tasktracker for entire cluster, for any/all reasons (admin only) */
  public int unblackListTaskTracker(String hostname) throws IOException;

  /**
   * Format current node labels and send it to jobclient 
   */
  public String getNodeLabels() throws IOException;
  
  /**
   * Ask JT to update current node labels.
   */
   ...
   * @throws IOException
   */
  public TaskCompletionEventList getTaskCompletionEventList(JobID jobid
