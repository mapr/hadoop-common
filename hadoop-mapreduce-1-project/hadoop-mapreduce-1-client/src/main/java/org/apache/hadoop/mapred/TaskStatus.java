...
public abstract class TaskStatus implements Writable, Cloneable {
  ...
  // FS to which task attempt logs will flow
  public static enum TaskLogFS {
    LOCAL ("file://"), 
    MAPRFS ("maprfs://");
    
    private String prefix;
    
    private TaskLogFS(String prefix) {
      this.prefix = prefix;
    }
  ...
  public TaskStatus(TaskAttemptID taskid, float progress, int numSlots,
    ...
    this.taskLogFs = CentralTaskLogUtil.CENTRAL_LOG ? TaskLogFS.MAPRFS : TaskLogFS.LOCAL;
  }
  ...
  public void setTaskLogFs(TaskLogFS taskLogFs) {
    this.taskLogFs = taskLogFs;
  }

  public TaskLogFS getTaskLogFs() {
    return taskLogFs;
  }
  ...
  void setFinishTime(long finishTime) {
    if(this.getStartTime() > 0 && finishTime > 0) {
      ...
      this.finishTime = this.startTime = 0;
      //Using String utils to get the stack trace.
      /* MapR: We may set time for unassigned tasks since prefetched 
       * attempts can be killed before they are launched. 
       */
      /*
  ...
  public void write(DataOutput out) throws IOException {
    ...
    WritableUtils.writeEnum(out, taskLogFs);
  }
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    this.taskLogFs = WritableUtils.readEnum(in, TaskLogFS.class);
   }
