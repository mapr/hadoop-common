...
class JobQueueTaskScheduler extends TaskScheduler {
  ...
  public synchronized void setConf(Configuration conf) {
    ...
    this.assignMultipleReduceTasks = 
      conf.getBoolean("mapreduce.fifoscheduler.assignmultiple.reducers", true);
  }
  ...
  public synchronized List<Task> assignTasks(TaskTracker taskTracker)
    ...
    /*
     * Assign more reduce tasks if reducer load is high. 
     * If cluster reduce capacity is 100 with 10 nodes and load is 0.5 
     * then each node receives max 5 reduce tasks in one single heartbeat.
     */
    if (assignMultipleReduceTasks) {
          ...
          for (JobInProgress job : jobQueue) {
            ...
            if (t != null) {
              ...
              if (exceededReducePadding) {
                break scheduleReducers;
              }
              // for next reducer scan all jobs again
              break;
            }
          }
        }
