...
class IFile {
  ...
  public static class Writer<K extends Object, V extends Object> {
    ...
    public Writer(Configuration conf, FSDataOutputStream output, 
        Class<K> keyClass, Class<V> valueClass,
        CompressionCodec codec, Counters.Counter writesCounter)
        throws IOException {
      if (TaskTracker.useMapRFs(conf)) {
        useMapRFs = true;
        this.checksumOut = new MapRIFileOutputStream(output);
        this.flushOnClose = false;
        if (codec != null) {
        ...
        this.checksumOut = new IFileOutputStream(output);
        if (codec != null) {
          this.compressor = CodecPool.getCompressor(codec);
          if (this.compressor != null) {
            this.compressor.reset();
    ...
    public void close() throws IOException {
      ...
      // write map out put counters at the end of file
      // TODO How do we move this into MapRIFileOutputStream? Probably doesn't make sense to do so.
      if (useMapRFs) {
        // Pass down total output size and how many raw bytes written so far
        MapOutputFileInfo info = new MapOutputFileInfo(rawOut, decompressedBytesWritten, 
                                                       (rawOut.getPos() - start));
        info.write();
      }
  ...
  public static class Reader<K extends Object, V extends Object> {
    ...
    public Reader(Configuration conf, FSDataInputStream input, long length, 
            CompressionCodec codec,
            Counters.Counter readsCounter) throws IOException {
      this(conf, input, length, codec, readsCounter, true);
    }
    ...
    public Reader(Configuration conf, FSDataInputStream input, long length, 
                  ...
                  boolean checkLength) throws IOException {
      if (conf != null && TaskTracker.useMapRFs(conf)) {
        readerInfo = new ReaderInfo();
        checksumIn = new MapRIFileInputStream(input, length, checkLength);
        if (codec != null) {
        ...
        readerInfo = new ReaderInfo();
        checksumIn = new IFileInputStream(input, length);
        if (codec != null) {
          decompressor = CodecPool.getDecompressor(codec);
          this.in = codec.createInputStream(checksumIn, decompressor);
        } else {
    ...
    public long getLength() { 
      return readerInfo.getLength();
    }
    
    public long getPosition() throws IOException { 
      return readerInfo.getPosition();
    }
  ...
  public static class InMemoryReader<K, V> extends Reader<K, V> {
    ...
    // closeInStream is ignored for InMemoryReader
    public void close(boolean closeInStream) {
      // Release
      memDataIn = null;
      buffer = null;
