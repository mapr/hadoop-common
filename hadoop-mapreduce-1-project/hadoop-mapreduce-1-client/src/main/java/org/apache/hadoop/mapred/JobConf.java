...
public class JobConf extends Configuration {
  ...
  private static final Pattern MAX_HEAP_PATTERN =
    Pattern.compile("\\s*-Xmx([0-9]+)([mgkMGK])\\s*");
  private static final Pattern MIN_HEAP_PATTERN =
    Pattern.compile("\\s*-Xms([0-9]+)([mgkMGK])\\s*");
  ...
  static{
    ...
    try {
      Configuration.addRemoteResource(
          new URI("maprfs:///var/mapr/cluster/mapred/mapred-site.xml"), true);
    } catch (Exception e) {} 
    */
  ...
  /* Virtual memory limits for tasks to be defined per job */
  ...
  /* MapR: Bug 3711 Physical memory limits for tasks to be defined per job */
  static final String MAPRED_JOB_MAP_MEMORY_PHYSICAL_MB_PROPERTY =
      "mapred.job.map.memory.physical.mb";

  static final String MAPRED_JOB_REDUCE_MEMORY_PHYSICAL_MB_PROPERTY =
      "mapred.job.reduce.memory.phyiscal.mb";
  ...
  public static final String DEFAULT_MAPRED_TASK_JAVA_OPTS = "-Xmx512m";
  ...
  /**
   * Configuration key for enabling the use of the ResourceCalculatorPlugin in
   * tasks.
   */
  public static final String MAPRED_TASK_RESOURCECALCULATORPLUGIN =
    "mapred.task.calculate.resource.usage";

  public static final boolean DEFAULT_MAPRED_TASK_RESOURCECALCULATORPLUGIN =
    true;
  ...
  public long getMemoryForMapTask() {
    ...
    return (value > 0)? value : DISABLED_MEMORY_LIMIT;
  }
  ...
  /* MapR: Bug 3711 adding new apis from trunk */
  public long getPhysicalMemoryForMapTask() {
    return normalizeMemoryConfigValue(
        getLong(JobConf.MAPRED_JOB_MAP_MEMORY_PHYSICAL_MB_PROPERTY,
          DISABLED_MEMORY_LIMIT));
  }
  
  public void setPhysicalMemoryForMapTask(long mem) {
    setLong(JobConf.MAPRED_JOB_MAP_MEMORY_PHYSICAL_MB_PROPERTY, mem);
  }
  ...
  public long getPhysicalMemoryForReduceTask() {
    return normalizeMemoryConfigValue(
        getLong(JobConf.MAPRED_JOB_REDUCE_MEMORY_PHYSICAL_MB_PROPERTY,
          DISABLED_MEMORY_LIMIT));
  }  

  public void setPhysicalMemoryForReduceTask(long mem) {
    setLong(JobConf.MAPRED_JOB_REDUCE_MEMORY_PHYSICAL_MB_PROPERTY, mem);
  }
  ...
  private static final Map<String, Double> suffixMultiplier =
  ...
  // return task's max heapsize in mb
  public int extractMaxHeapSize(String opt) {
    String val = null;
    int mem = -1;
    String []tokens = opt.split(" ");
    for (int i=tokens.length - 1; i >= 0; i--) {
      String token = tokens[i].trim();
      Matcher m = MAX_HEAP_PATTERN.matcher(token);
      if (m.matches()) {
        return (int) Math.ceil(Integer.valueOf(m.group(1)) *
            suffixMultiplier.get(m.group(2).toLowerCase()));
      }
    }
    return mem;
  }

  // return task's min heapsize in mb
  public int extractMinHeapSize(String opt) {
    String val = null;
    int mem = -1;
    String []tokens = opt.split(" ");
    for (int i=tokens.length - 1; i >= 0; i--) {
      String token = tokens[i].trim();
      Matcher m = MIN_HEAP_PATTERN.matcher(token);
      if (m.matches()) {
        return (int) Math.ceil(Integer.valueOf(m.group(1)) *
            suffixMultiplier.get(m.group(2).toLowerCase()));
      }
  ...
  public int getMapTaskMaxHeapSize() {
    return extractMaxHeapSize(this.get(MAPRED_MAP_TASK_JAVA_OPTS,
                                    this.get(MAPRED_TASK_JAVA_OPTS)));
  }
  
  public int getReduceTaskMaxHeapSize() {
    return extractMaxHeapSize(this.get(MAPRED_REDUCE_TASK_JAVA_OPTS,
                                    this.get(MAPRED_TASK_JAVA_OPTS)));
  }
  
  public int getMapTaskMinHeapSize() {
    return extractMinHeapSize(this.get(MAPRED_MAP_TASK_JAVA_OPTS,
                                    this.get(MAPRED_TASK_JAVA_OPTS)));
  }
  
  public int getReduceTaskMinHeapSize() {
    return extractMinHeapSize(this.get(MAPRED_REDUCE_TASK_JAVA_OPTS,
                                    this.get(MAPRED_TASK_JAVA_OPTS)));
  }
}
